...
void insertarAlInicio(struct Lista & tad, struct NodoArbol * nodoArbol) {

    struct Nodo * nuevoNodo = crearNodo(nodoArbol, tad.cabeza);
    tad.cabeza = nuevoNodo;
    tad.longitud++;
}

/*Crea un nuevo nodo con los datos dados como parámetros*/
struct Nodo * crearNodo(struct NodoArbol * nodoArbol, struct Nodo * siguiente) {
...
void insertarAlFinal(struct Lista & tad, struct NodoArbol * nodoArbol) {

    struct Nodo * ultimoNodo = obtenerUltimoNodo(tad);
    struct Nodo * nuevoNodo = crearNodo(nodoArbol, nullptr);
