...
void insertarAlInicioArchivo(const char * nombArchText, struct Lista & tad) {
    ...
    while (true) {
        archText>>elemento.anho; //lee el elemento del archivo
...
struct Nodo * obtenerNodoAnterior(const struct Lista & tad, const struct Elemento & elemento) {
    ...
    struct Nodo * recorrido = tad.cabeza;
    while ((recorrido != nullptr) and (recorrido->elemento.anho < elemento.anho)) {
        anterior = recorrido;
        recorrido = recorrido->siguiente;
    }
    return anterior;
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento.anho << " " << recorrido->elemento.cantidad;
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
