...
struct Nodo * crearNodo(struct NodoArbol * nodoArbol, struct Nodo * siguiente, int nivel) {
...
void insertarAlFinal(struct Lista & tad, struct NodoArbol * nodoArbol, int nivel) {

    //struct Nodo * ultimoNodo = obtenerUltimoNodo(lista);
    struct Nodo * ultimoNodo = tad.cola; /*obtiene el último nodo*/
    struct Nodo * nuevoNodo = crearNodo(nodoArbol, nullptr, nivel);
...
void retornaCabeza(const struct Lista & tad, struct NodoArbol *& nodoArbol, int & nivel) {
    nodoArbol = tad.cabeza->nodoArbol;
    nivel = tad.cabeza->nivel;
}
