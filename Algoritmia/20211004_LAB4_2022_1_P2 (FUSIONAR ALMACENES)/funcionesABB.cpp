    ...
    if(esNodoVacio(raiz))
        ...
        if(raiz->elemento.numLote > elemento.numLote)
            ...
            if(raiz->elemento.numLote < elemento.numLote)
                ...
                cout << "El elemento " << elemento.numLote << "Ya se encuentra en el árbol" << endl;
}
...
bool buscarRecursivo(struct NodoArbol * nodo, const struct Elemento & elemento){
    if(esNodoVacio(nodo))
        return false;
    if(comparaABB(nodo->elemento.numLote, elemento.numLote) == 0)
        return true;
    if(comparaABB(nodo->elemento.numLote, elemento.numLote) == 1)
...
struct NodoArbol * eliminarRecursivo(struct NodoArbol * nodo, const struct Elemento & elemento){
    ...
    if(nodo->elemento.numLote > elemento.numLote)
